...
def save_profile_to_db(user_id, user_data, bot):
    try:
        ...
        c.execute('''INSERT OR REPLACE INTO profiles 
                     (user_id, username, first_name, game, rank, roles, description, timestamp)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                  (user_id,
...
def ban_profile(user_id):
    conn = sqlite3.connect('teammates.db')
    c = conn.cursor()
    c.execute("UPDATE profiles SET is_banned = 1 WHERE user_id = ?", (user_id,))
    c.execute("UPDATE reports SET status = 'resolved' WHERE profile_id = ?", (user_id,))
    conn.commit()
    conn.close()
...
def migrate_db():
    try:
        ...
        c.execute('''INSERT INTO profiles_new 
                     (user_id, username, first_name, game, rank, roles, description, timestamp)
                     SELECT user_id, username, first_name, game, rank, roles, description, timestamp
                     FROM profiles''')
